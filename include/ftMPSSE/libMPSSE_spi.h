...
enum SPI_ConfigOptionMode {
	/*
		In addition, the SPI interface has 4 unique modes of clock phase (CPHA) and clock polarity (CPOL), known
		as Mode 0, Mode 1, Mode 2 and Mode 3. Table 1 summarizes these modes.

		For CPOL = 0, the base (inactive) level of SCLK is 0.
			In this mode:
				When CPHA = 0, data will be read in on the rising edge of SCLK, and data will be clocked out on the
			falling edge of SCLK.
				When CPHA = 1, data will be read in on the falling edge of SCLK, and data will clocked out on the
			rising edge of SCLK
		For CPOL =1, the base (inactive) level of SCLK is 1.
			In this mode:
				When CPHA = 0, data will be read in on the falling edge of SCLK, and data will clocked out on the
			rising edge of SCLK
				When CPHA = 1, data will be read in on the rising edge of SCLK, and data will be clocked out on the
		falling edge of SCLK.
...
#ifndef _MSC_VER
typedef unsigned char	ftdiBool;
#endif
...
FTDI_API FT_STATUS SPI_IsBusy(FT_HANDLE handle, ftdiBool *state);
